...
const Discover = () => {
  ...
  const activeTopicStyle =
    "xl:border-2 hover:bg-primary xl:border-gray-[#F51997] px-3 py-2 rounded xl:rounded-full flex item-center gap-2 justify-center cursor-pointer text-[#F51997]";
      ...
      <div className="flex gap-3 flex-wrap">
        {topics.map((topic) => (
          <Link href={`/?topic=${topic.name}`} key={topic.name}>
            <div
              className={
                subTopic === topic.name ? activeTopicStyle : topicStyle
              }
            >
